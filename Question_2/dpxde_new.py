...
boundary_condition_u = dde.DirichletBC(
    spatial_domain,
    lambda x: 1.0 if any(dde.utils.isclose(x[0], 1)) and dde.utils.isclose(x[1], 1) else 0.0,
    lambda x, on_boundary: on_boundary and not dde.utils.isclose(x[1], 1),
    component=0,
)

# Modify the boundary condition for v at the left, right, and bottom sides
boundary_condition_v = dde.DirichletBC(
    spatial_domain,
    lambda x: 0.0 if np.allclose(x[1], 0) or np.allclose(x[0], 0) or np.allclose(x[0], 1) else 1.0,
    lambda x, on_boundary: (
        on_boundary
        and not np.allclose(x[1], 1)
        and not np.allclose(x[1], 0)
    ),
    component=1,
)
