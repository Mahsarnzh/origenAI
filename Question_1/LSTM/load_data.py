...
def load_and_process_data(data_folder="./origen_interview_data/"):
    Inpu_nms = {"DEPTH", "TRANY", "PORV", "TRANX", "PERMZ", "TRANZ", "PORO", "SATNUM", "PERMY", "PERMX"}
    data = {}
    ...
    df = pd.concat([
        pd.concat([
            pd.DataFrame({"name": [k, ] * v.shape[0]}, index=v.index),
            v.rename(columns={c: c.split("_")[-1] for c in v.columns}),
        ], axis=1)
        for k, v in dfs.items() if k in Inpu_nms
    ])
