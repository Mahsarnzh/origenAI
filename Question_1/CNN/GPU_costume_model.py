...
if len(physical_devices) == 0:
    print("No GPU devices available. TensorFlow will use CPU.")
else:
    print("GPU devices available. TensorFlow will use GPU.")

# Configure TensorFlow session for GPU
...
# Train the model
model.fit(
    x={'input_cnn1': X_train_fc_split, 'input_cnn2': X_train_cnn_normalized[train_indices]},
    y=y_scaled[train_indices],
    epochs=20, batch_size=32,
    validation_data=({'input_cnn1': X_val_fc_split, 'input_cnn2': X_train_cnn_normalized[val_indices]}, y_scaled[val_indices])
)
